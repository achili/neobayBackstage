  ...
  <div class="row">
    ...
    <div class="col-xs-12">
      ...
      <div>
        ...
        <span v-if="financePayInfo.archive">
						解封将恢复归档前的状态。
					</span>
        <span v-else>
						归档后的将不能操作，默认不出现在列表中。
      ...
      <div class="mt10">
        <SubmitButton v-if="financePayInfo.archive" title="解封" :loading="financePayInfo.loading" :callback="archive"
                      submitClass="btn btn-warning" submitIcon="fa fa-unlock"/>
        <SubmitButton v-else title="归档" :loading="financePayInfo.loading" :callback="archive"
...
<script>
  ...
  export default{
    ...
    methods: {
      archive(){
        let that = this;
        this.financePayInfo.httpArchive(this.reviewReason, function (response) {
