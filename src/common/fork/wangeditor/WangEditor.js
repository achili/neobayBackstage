...
// 定义构造函数
	(function (window, $) {
		...
		//二次开发全屏功能
		E.fullScreenClick = null;
		E.fullScreenClickSelected = null;
...
// editor api
	_e(function (E, $) {
		...
		E.fn.updateValue = function () {
			let editor = this;
			let $valueContainer = editor.$valueContainer;
			let $txt = editor.txt.$txt;
		...
		E.fn.getInitValue = function () {
			...
			let $valueContainer = editor.$valueContainer;
			let currentValue = '';
			let nodeName = editor.valueNodeName;
			if (nodeName === 'div') {
		...
		E.fn.hideDropPanelAndModal = function () {
			...
			$.each(menus, function (k, menu) {
				let m = menu.dropPanel || menu.dropList || menu.modal;
				if (m && m.hide) {
					m.hide();
				}
			});
...
// selection range API
	_e(function (E, $) {
		...
		E.fn.getRangeElem = ieRange ? emptyFn : function (range) {
				range = range || this.currentRange();
				let dom = range.commonAncestorContainer;
		...
		E.fn.initSelection = ieRange ? emptyFn : function () {
				...
				let range;
				let $txt = editor.txt.$txt;
				let $firstChild = $txt.children().first();
		...
		E.fn.setRangeByElem = ieRange ? emptyFn : function (elem) {
				let editor = this;
				let txtElem = editor.txt.$txt.get(0);
				if (!elem || !$.contains(txtElem, elem)) {
					return;
				}

				// 找到elem的第一个 textNode 和 最后一个 textNode
				let firstTextNode = elem.firstChild;
				while (firstTextNode) {
				...
				let lastTextNode = elem.lastChild;
				while (lastTextNode) {
...
// editor command API
	_e(function (E, $) {
		...
		E.fn.commandForElem = function (elemOpt, e, commandName, commandValue, callback) {
			...
			// 查询elem
			let rangeElem = this.getRangeElem();
...
// dom selector
	_e(function (E, $) {
		...
		E.fn.getLegalTags = function (elem) {
			let legalTags = this.config.legalTags;
			if (!legalTags) {
...
// undo redo
	_e(function (E, $) {
		...
		function _handle(editor, data, type) {
			...
			// let range2 = range.cloneRange && range.cloneRange();
			let val = data.val;
			let html = editor.txt.$txt.html();
		...
		E.fn.undoRecord = function () {
			...
			let $txt = editor.txt.$txt;
			let val = $txt.html();
			let undoList = _getUndoList(editor);
			let redoList = _getRedoList(editor);
			let currentVal = undoList.length ? undoList[0] : '';
...
// 暴露给用户的 API
	_e(function (E, $) {
		...
		E.fn.destroy = function () {
			let self = this;
			let $valueContainer = self.$valueContainer;
			let $editorContainer = self.$editorContainer;
			let valueNodeName = self.valueNodeName;
		...
		E.fn.undestroy = function () {
			...
			let $valueContainer = self.$valueContainer;
			let $editorContainer = self.$editorContainer;
			let $menuContainer = self.menuContainer.$menuContainer;
			let valueNodeName = self.valueNodeName;
...
// menuContainer 构造函数
	_e(function (E, $) {
		...
		// 定义构造函数
		let MenuContainer = function (editor) {
			this.editor = editor;
			this.init();
		};
...
// MenuContainer.fn bind fn
	_e(function (E, $) {
		...
		let MenuContainer = E.MenuContainer;

		// 初始化
		MenuContainer.fn.init = function () {
			let self = this;
			let $menuContainer = $('<div class="wangEditor-menu-container clearfix"></div>');
...
// MenuContainer.fn API
	_e(function (E, $) {
		...
		MenuContainer.fn.render = function () {
			let $menuContainer = this.$menuContainer;
			let $editorContainer = this.editor.$editorContainer;
		...
		MenuContainer.fn._addOneMenu = function (menu) {
			let $menuNormal = menu.$domNormal;
			let $menuSelected = menu.$domSelected;

			let $menuGroup = this.$currentGroup;
			let $item = $('<div class="menu-item clearfix"></div>');
			$menuSelected.hide();
...
// Menu.fn 初始化绑定的事件
	_e(function (E, $) {
		...
		Menu.fn.initUI = function () {
			let editor = this.editor;
			let uiConfig = editor.UI.menus;
			let menuId = this.id;
			let menuUI = uiConfig[menuId];
...
// Menu.fn API
	_e(function (E, $) {
		...
		Menu.fn.render = function (groupIdx) {
			...
			let editor = this.editor;
			let menuContainer = editor.menuContainer;
			let $menuItem = menuContainer.appendMenu(groupIdx, this);
			let onRender = this.onRender;
		...
		Menu.fn._renderTip = function ($menuItem) {
			...
			let $tip = $('<div class="menu-tip"></div>');
			// let $triangle = $('<i class="tip-triangle"></i>'); // 小三角
			...
			if (!self.tipWidth) {
				...
				editor.ready(function () {
					let editor = this;
					let titleWidth = $tempDiv.outerWidth() + 5; // 多出 5px 的冗余
					let currentWidth = $tip.outerWidth();
					let currentMarginLeft = parseFloat($tip.css('margin-left'), 10);
					// 计算完，拿到数据，则弃用
					$tempDiv.remove();
					$tempDiv = null;
		...
		Menu.fn.bindEvent = function () {
			...
			let $domNormal = self.$domNormal;
			let $domSelected = self.$domSelected;

			// 试图获取该菜单定义的事件（未selected），没有则自己定义
			let clickEvent = self.clickEvent;
			if (!clickEvent) {
				clickEvent = function (e) {
					// -----------dropPanel dropList modal-----------
					let dropObj = self.dropPanel || self.dropList || self.modal;
			...
			let clickEventSelected = self.clickEventSelected || clickEvent;
		...
		Menu.fn.updateSelected = function () {
			...
			// 试图获取用户自定义的判断事件
			let updateSelectedEvent = self.updateSelectedEvent;
			if (!updateSelectedEvent) {
				...
				updateSelectedEvent = function () {
					...
					let commandValue = self.commandValue;
			...
			// 获取结果
			let result = updateSelectedEvent.call(self);
			result = !!result;
		...
		Menu.fn.activeStyle = function (active) {
			...
			let $dom = this.$domNormal;
			let $domSelected = this.$domSelected;
...
// dropList 构造函数
	_e(function (E, $) {
		...
		// 定义构造函数
		let DropList = function (editor, menu, opt) {
			this.editor = editor;
			this.menu = menu;
...
// dropList fn bind
	_e(function (E, $) {
		...
		DropList.fn.initDOM = function () {
			let self = this;
			let data = self.data;
			let tpl = self.tpl || '<span>{#title}</span>';
			let $list = $('<div class="wangEditor-drop-list clearfix"></div>');
		...
		DropList.fn.bindEvent = function () {
			...
			let editor = self.editor;
			let menu = self.menu;
			let commandName = menu.commandName;
			let selectorForELemCommand = self.selectorForELemCommand;
			let $list = self.$list;

			// 执行事件前后的钩子函数
			let beforeEvent = self.beforeEvent;
			let afterEvent = self.afterEvent;
			...
			$list.on('click', 'a[commandValue]', function (e) {
				...
				// 执行命令
				let commandValue = $(e.currentTarget).attr('commandValue');
...
// dropListfn api
	_e(function (E, $) {
		...
		DropList.fn._position = function () {
			...
			let editor = self.editor;
			let menu = self.menu;
			let $menuContainer = editor.menuContainer.$menuContainer;
			let $menuDom = menu.selected ? menu.$domSelected : menu.$domNormal;
			// 注意这里的 offsetParent() 要返回 .menu-item 的 position
			// 因为 .menu-item 是 position:relative
			let menuPosition = $menuDom.offsetParent().position();

			// 取得 menu 的位置、尺寸属性
			let menuTop = menuPosition.top;
			let menuLeft = menuPosition.left;
			let menuHeight = $menuDom.offsetParent().height();
			let menuWidth = $menuDom.offsetParent().width();
			...
			// 取得 $txt 的尺寸
			let txtWidth = editor.txt.$txt.outerWidth();

			// ------------开始计算-------------

			// 初步计算 list 位置属性
			let top = menuTop + menuHeight;
			let left = menuLeft + menuWidth / 2;
			let marginLeft = 0 - menuWidth / 2;

			// 如果超出了有边界，则要左移（且和右侧有间隙）
			let valWithTxt = (left + listWidth) - txtWidth;
			if (valWithTxt > -10) {
...
// dropPanel 构造函数
	_e(function (E, $) {
		...
		// 定义构造函数
		let DropPanel = function (editor, menu, opt) {
			this.editor = editor;
			this.menu = menu;
...
// dropPanel fn bind
	_e(function (E, $) {
		...
		DropPanel.fn.initDOM = function () {
			...
			let $panel = $('<div class="wangEditor-drop-panel clearfix"></div>');
			let $triangle = $('<div class="tip-triangle"></div>');
		...
		DropPanel.fn.initHideEvent = function () {
			...
			// 获取 panel elem
			let thisPanle = self.$panel.get(0);
...
// dropPanel fn api
	_e(function (E, $) {
		...
		DropPanel.fn._position = function () {
			...
			let top = menuTop + menuHeight;
			let left = menuLeft + menuWidth / 2;
			let marginLeft = 0 - panelWidth / 2;
			let marginLeft2 = marginLeft;  // 下文用于和 marginLeft 比较，来设置三角形tip的位置
			...
			let valWithTxt = (left + panelWidth + marginLeft) - txtWidth;
			if (valWithTxt > -10) {
				marginLeft = marginLeft - valWithTxt - 10;
			}
...
// modal 构造函数
	_e(function (E, $) {
		...
		// 定义构造函数
		let Modal = function (editor, menu, opt) {
			this.editor = editor;
			this.menu = menu;
			this.$content = opt.$content;
...
// modal fn bind
	_e(function (E, $) {
		...
		Modal.fn.initDom = function () {
			let self = this;
			let $content = self.$content;
			let $modal = $('<div class="wangEditor-modal"></div>');
			let $close = $('<div class="wangEditor-modal-close"><i class="wangeditor-menu-img-cancel-circle"></i></div>');
...
// modal fn api
	_e(function (E, $) {
		...
		Modal.fn._position = function () {
			let self = this;
			let $modal = self.$modal;
			let top = $modal.offset().top;
			let width = $modal.outerWidth();
			let height = $modal.outerHeight();
			let marginLeft = 0 - (width / 2);
			let marginTop = 0 - (height / 2);
			let sTop = E.$window.scrollTop();
...
// Txt.fn bind fn
	_e(function (E, $) {
		...
		Txt.fn.init = function () {
			...
			let editor = self.editor;
			let $valueContainer = editor.$valueContainer;
			let currentValue = editor.getInitValue();
			let $txt;
		...
		Txt.fn.contentEmptyHandle = function () {
			...
			$txt.on('keydown', function (e) {
				...
				let txtHtml = $.trim($txt.html().toLowerCase());
				if (txtHtml === '<p><br></p>') {
		...
		Txt.fn.bindEnterForDiv = function () {
			let tags = E.config.legalTags; // 配置中编辑器要求的合法标签，如 p head table blockquote ul ol 等
			let self = this;
			let editor = self.editor;
			let $txt = self.$txt;
			...
			function divHandler() {
				...
				let $pElem = $('<p>' + $keydownDivElem.html() + '</p>');
			...
			$txt.on('keydown keyup', function (e) {
				...
				// 查找合法标签
				let rangeElem = editor.getRangeElem();
				let targetElem = editor.getLegalTags(rangeElem);
				let $targetElem;
				let $pElem;
		...
		Txt.fn.bindPasteFilter = function () {
			let self = this;
			let editor = self.editor;
			let resultHtml = '';  //存储最终的结果
			let $txt = self.$txt;
			let legalTags = editor.config.legalTags;
			let legalTagArr = legalTags.split(',');
			...
			$txt.on('paste', function (e) {
				...
				let currentNodeName = editor.getRangeElem().nodeName;
			...
			function removeAttrs(elem) {
				...
				let attrNames = [];
				let exception = ['href', 'target', 'src', 'alt', 'rowspan', 'colspan']; //例外情况
		...
		Txt.fn.bindFormatText = function () {
			...
			let editor = self.editor;
			let $txt = self.$txt;
			let legalTags = E.config.legalTags;
			let legalTagArr = legalTags.split(',');
			let length = legalTagArr.length;
			let regArr = [];
		...
		Txt.fn.bindHtml = function () {
			...
			let $txt = self.$txt;
			let $valueContainer = editor.$valueContainer;
			let valueNodeName = editor.valueNodeName;
...
// Txt.fn api
	_e(function (E, $) {
		...
		let txtChangeEventNames = 'propertychange change click keyup input paste';
		...
		Txt.fn.initHeight = function () {
			let editor = this.editor;
			let $txt = this.$txt;
			let valueContainerHeight = editor.$valueContainer.height();
			let menuHeight = editor.menuContainer.height();
			let txtHeight = valueContainerHeight - menuHeight;
		...
		Txt.fn.initMaxHeight = function (txtHeight, menuHeight) {
			...
			if (window.getComputedStyle && 'max-height' in window.getComputedStyle($txt.get(0))) {
				...
				let maxHeight = parseInt(editor.$valueContainer.css('max-height'));
				if (isNaN(maxHeight)) {
					return;
				}
		...
		Txt.fn.wrapImgAndText = function () {
			let $txt = this.$txt;
			let $imgs = $txt.children('img');
			let txt = $txt[0];
			let childNodes = txt.childNodes;
			let childrenLength = childNodes.length;
			let i, childNode, p;
		...
		Txt.fn.showHeightOnHover = function () {
			...
			let menuContainer = editor.menuContainer;
			let $txt = this.$txt;
			let $tip = $('<i class="height-tip"><i>');
			let isTipInTxt = false;
			...
			function addAndShowTip($target) {
				...
				let txtTop = $txt.position().top;
				let txtHeight = $txt.outerHeight();

				let height = $target.height();
				let top = $target.position().top;
				let marginTop = parseInt($target.css('margin-top'), 10);
				let paddingTop = parseInt($target.css('padding-top'), 10);
				let marginBottom = parseInt($target.css('margin-bottom'), 10);
				let paddingBottom = parseInt($target.css('padding-bottom'), 10);

				// 计算初步的结果
				let resultHeight = height + paddingTop + marginTop + paddingBottom + marginBottom;
				let resultTop = top + menuContainer.height();
...
// 增加menus
	_e(function (E, $) {
		...
		E.fn.addMenus = function () {
			let editor = this;
			let menuIds = editor.config.menus;
	...
	_e(function (E, $) {
		...
		E.createMenu(function (check) {
			...
			menu.clickEventSelected = function (e) {
				let isRangeEmpty = editor.isRangeEmpty();
				if (!isRangeEmpty) {
					// 如果选区有内容，则执行基础命令
...
// forecolor 菜单
	_e(function (E, $) {
		...
		E.createMenu(function (check) {
			let menuId = 'forecolor';
			if (!check(menuId)) {
				return;
			}
			let editor = this;
			let lang = editor.config.lang;
			let configColors = editor.config.colors;
			...
			$content.on('click', 'a[commandValue]', function (e) {
				// 执行命令
				let $elem = $(this);
				let commandValue = $elem.attr('commandValue');
		...
		E.createMenu(function (check) {
			...
			menu.clickEvent = function (e) {
				...
				function commandFn() {
					...
					let rangeElem;
					let pElem, $pElem;
					let quoteElem, $quoteElem;
					let listElem, $listElem;
	...
	_e(function (E, $) {
		...
		E.createMenu(function (check) {
			...
			function updateValue() {
				let $code = menu.$codeTextarea;
				let $txt = editor.txt.$txt;
				let value = $.trim($code.val()); // 取值
			...
			menu.clickEvent = function (e) {
				...
				let editor = self.editor;
				let $txt = editor.txt.$txt;
				let txtOuterHeight = $txt.outerHeight();
				let txtHeight = $txt.height();

				if (!self.$codeTextarea) {
					self.$codeTextarea = $('<textarea class="code-textarea"></textarea>');
				}
				let $code = self.$codeTextarea;
				$code.css({
					height: txtHeight,
	...
	_e(function (E, $) {
		...
		E.createMenu(function (check) {
			...
			menu.clickEvent = function (e) {
				...
				let currentQuote = editor.getSelfOrParentByName(rangeElem, 'blockquote');
				let $quote;
			...
			editor.ready(function () {
				let editor = this;
				let $txt = editor.txt.$txt;
				let isPrevEnter = false;  // 是不是刚刚在quote中按了 enter 键
				$txt.on('keydown', function (e) {
					if (e.keyCode !== 13) {
						// 不是 enter 键
						isPrevEnter = false;
					...
					let currentRangeElem = editor.getRangeElem();
					let $currentRangeElem = $(currentRangeElem);
...
// 字体 菜单
	_e(function (E, $) {
		...
		E.createMenu(function (check) {
			let menuId = 'fontfamily';
			if (!check(menuId)) {
				return;
			}
			let editor = this;
			let lang = editor.config.lang;
			let configFamilys = editor.config.familys;
			...
			// 创建droplist
			let tpl = '<span style="font-family:{#commandValue};">{#title}</span>';
...
// 字号 菜单
	_e(function (E, $) {
		E.createMenu(function (check) {
			...
			let editor = this;
			let lang = editor.config.lang;
			let configSize = editor.config.fontsizes;
			...
			// 创建droplist
			let tpl = '<span style="font-size:{#title};">{#title}</span>';
			menu.dropList = new E.DropList(editor, menu, {
				data: data,
				tpl: tpl,
...
// link 菜单
	_e(function (E, $) {
		...
		E.createMenu(function (check) {
			...
			// 创建 dropPanel
			let $content = $('<div></div>');
			let $div1 = $('<div style="margin:20px 10px;" class="clearfix"></div>');
			let $div2 = $div1.clone();
			let $div3 = $div1.clone().css('margin', '0 10px');
			let $textInput = $('<input type="text" class="block" placeholder="' + lang.text + '"/>');
			let $urlInput = $('<input type="text" class="block" placeholder="' + lang.link + '"/>');
			let $btnSubmit = $('<button class="right">' + lang.submit + '</button>');
			let $btnCancel = $('<button class="right gray">' + lang.cancel + '</button>');
			...
			$btnSubmit.click(function (e) {
				e.preventDefault();
				let rangeElem = editor.getRangeElem();
				let targetElem = editor.getSelfOrParentByName(rangeElem, 'a');
				let isRangeEmpty = editor.isRangeEmpty();

				let $linkElem, linkHtml;
				let commandFn, callback;
				let $txt = editor.txt.$txt;
				let $oldLinks, $newLinks;
				let uniqId = 'link' + E.random();

				// 获取数据
				let url = $.trim($urlInput.val());
				let text = $.trim($textInput.val());
	...
	_e(function (E, $) {
		...
		E.createMenu(function (check) {
			...
			menu.clickEvent = function (e) {
				...
				let rangeElem = editor.getRangeElem();
				let aElem = editor.getSelfOrParentByName(rangeElem, 'a');
				if (!aElem) {
					// 不在 a 之内，返回
					e.preventDefault();
					return;
...
// table 菜单
	_e(function (E, $) {
		...
		E.createMenu(function (check) {
			...
			// dropPanel 内容
			let $content = $('<div style="font-size: 14px; color: #666; text-align:right;"></div>');
			let $table = $('<table class="choose-table" style="margin-bottom:10px;margin-top:5px;">');
			let $row = $('<span>0</span>');
			let $rowspan = $('<span> 行 </span>');
			let $col = $('<span>0</span>');
			let $colspan = $('<span> 列</span>');
			let $tr;
			let i, j;
			...
			$table.on('mouseenter', 'td', function (e) {
				let $currentTd = $(e.currentTarget);
				let currentTdIndex = $currentTd.attr('index');
				let $currentTr = $currentTd.parent();
				let currentTrIndex = $currentTr.attr('index');
			...
			$table.on('click', 'td', function (e) {
				...
				let rownum = parseInt(currentTrIndex, 10);
				let colnum = parseInt(currentTdIndex, 10);
...
// emotion 菜单
	_e(function (E, $) {
		...
		E.createMenu(function (check) {
			...
			let configEmotions = config.emotions;
			let emotionsShow = config.emotionsShow;
			...
			function insertEmotionImgs(data, $tabContent) {
				...
				$.each(data, function (k, emotion) {
					let src = emotion.icon || emotion.url;
					let value = emotion.value || emotion.title;
					// 通过配置 editor.config.emotionsShow 的值来修改插入到编辑器的内容（图片/value）
					let commandValue = emotionsShow === 'icon' ? src : value;
					let $command = $('<a href="#" commandValue="' + commandValue + '"></a>');
					let $img = $('<img>');
			...
			// 拼接 dropPanel 内容
			let $panelContent = $('<div class="panel-tab"></div>');
			let $tabContainer = $('<div class="tab-container"></div>');
			let $contentContainer = $('<div class="content-container emotion-content-container"></div>');
			$.each(configEmotions, function (k, emotion) {
				let title = emotion.title;
				let data = emotion.data;

				E.log('正在处理 ' + title + ' 表情的数据...');

				// 增加该组表情的tab和content
				let $tab = $('<a href="#">' + title + ' </a>');
				$tabContainer.append($tab);
				let $tabContent = $('<div class="content"></div>');
			...
			$contentContainer.on('click', 'a[commandValue]', function (e) {
				...
				let commandValue = $a.attr('commandValue');
				let img;
...
// img 菜单
	_e(function (E, $) {
		...
		E.createMenu(function (check) {
			...
			// tab
			let $uploadTab = $('<a href="#">上传图片</a>');
			let $linkTab = $('<a href="#">网络图片</a>');
			$tabContainer.append($uploadTab).append($linkTab);

			// 上传图片 content
			let $uploadContent = $('<div class="content"></div>');
			$contentContainer.append($uploadContent);

			// 网络图片 content
			let $linkContent = $('<div class="content"></div>');
			...
			menu.dropPanel = new E.DropPanel(editor, menu, {
				...
				onRender: function () {
					...
					let init = editor.config.customUploadInit;
			...
			editor.ready(function () {
				let editor = this;
				let config = editor.config;
				let uploadImgUrl = config.uploadImgUrl;
				let customUpload = config.customUpload;
				let linkImg = config.hideLinkImg;
				let $uploadImgPanel;
		...
		function linkContentHandler(editor, menu, $linkContent) {
			let lang = editor.config.lang;
			let $urlContainer = $('<div style="margin:20px 10px 10px 10px;"></div>');
			let $urlInput = $('<input type="text" class="block" placeholder="http://"/>');
			...
			$btnSubmit.click(function (e) {
				...
				let imgHtml = '<img style="max-width:100%;" src="' + url + '"/>';
...
// video 菜单
	_e(function (E, $) {
		...
		E.createMenu(function (check) {
			...
			let editor = this;
			let lang = editor.config.lang;
			let reg = /^<(iframe)|(embed)/i;  // <iframe... 或者 <embed... 格式
			...
			// 创建 panel 内容
			let $content = $('<div></div>');
			let $linkInputContainer = $('<div style="margin:20px 10px;"></div>');
			let $linkInput = $('<input type="text" class="block" placeholder=\'格式如：<iframe src="..." frameborder=0 allowfullscreen></iframe>\'/>');
			$linkInputContainer.append($linkInput);
			let $sizeContainer = $('<div style="margin:20px 10px;"></div>');
			let $widthInput = $('<input type="text" value="640" style="width:50px;text-align:center;"/>');
			let $heightInput = $('<input type="text" value="498" style="width:50px;text-align:center;"/>');
			...
			let $howToCopy = $('<a href="http://www.kancloud.cn/wangfupeng/wangeditor2/134973" target="_blank" style="display:inline-block;margin-top:10px;margin-left:10px;color:#999;">如何复制视频链接？</a>');
			let $btnSubmit = $('<button class="right">' + lang.submit + '</button>');
			let $btnCancel = $('<button class="right gray">' + lang.cancel + '</button>');
...
// location 菜单
	_e(function (E, $) {
		...
		E.createMenu(function (check) {
			...
			// 搜索框
			let $inputContainer = $('<div style="margin:10px 0;"></div>');
			let $cityInput = $('<input type="text"/>');
			$cityInput.css({
				width: '80px',
				'text-align': 'center'
			});
			...
			let $clearBtn = $('<button class="right link">' + lang.clearLocation + '</button>');
			...
			// 地图
			let $map = $('<div id="' + mapData.mapContainerId + '"></div>');
			$map.css({
				height: '260px',
				width: '100%',
				position: 'relative',
			...
			let $mapLoading = $('<span>' + lang.loading + '</span>');
			$mapLoading.css({
				position: 'absolute',
				width: '100px',
				'text-align': 'center',
			...
			// 按钮
			let $btnContainer = $('<div style="margin:10px 0;"></div>');
			let $btnSubmit = $('<button class="right">' + lang.submit + '</button>');
			let $btnCancel = $('<button class="right gray">' + lang.cancel + '</button>');
			let $checkLabel = $('<label style="display:inline-block;margin-top:10px;color:#666;"></label>');
	...
	_e(function (E, $) {
		...
		E.createMenu(function (check) {
			...
			menu.clickEventSelected = function (e) {
				...
				let rangeElem = editor.getRangeElem();
				let targetElem = editor.getSelfOrParentByName(rangeElem, 'pre');
				let $targetElem;
				let className;
				if (targetElem) {
					// 确定找到 pre 之后，再找 code
			...
			function contentHandle($content) {
				...
				// 确定按钮
				let codeTpl = '<pre style="max-width:100%;overflow-x:auto;"><code{#langClass}>{#content}</code></pre>';
				$btnSubmit.click(function (e) {
					e.preventDefault();
					...
					let lang = $langSelect ? $langSelect.val() : ''; // 获取高亮语言
					let langClass = '';
					let doHightlight = function () {
					...
					if (!menu.selected) {
						// 拼接html
						let html = codeTpl.replace('{#langClass}', langClass).replace('{#content}', val);
			...
			$txt.on('keydown', function (e) {
				...
				let rangeElem = editor.getRangeElem();
				let targetElem = editor.getSelfOrParentByName(rangeElem, 'code');
				if (!targetElem) {
					return;
				}
...
// 全屏 菜单
	_e(function (E, $) {
		...
		// 记录全屏时的scrollTop
		let scrollTopWhenFullScreen;

		E.createMenu(function (check) {
			let menuId = 'fullscreen';
			if (!check(menuId)) {
				return;
			}
			let editor = this;
			let $txt = editor.txt.$txt;
			let config = editor.config;
			let zIndexConfig = config.zindex || 10000;
			let lang = config.lang;
			...
			menu.clickEvent = function (e) {
				...
				if (typeof editor.fullScreenClick === "function") {
					editor.fullScreenClick();
				}
			...
			menu.clickEventSelected = function (e) {
				...
				if (typeof editor.fullScreenClickSelected === "function") {
					editor.fullScreenClickSelected();
				}
...
// 上传图片事件
	_e(function (E, $) {
		...
		E.plugin(function () {
			let editor = this;
			let fns = editor.config.uploadImgFns; // editor.config.uploadImgFns = {} 在config文件中定义了
			...
			fns.onload || (fns.onload = function (resultText, xhr) {
				...
				let editor = this;
				let originalName = editor.uploadImgOriginalName || '';  // 上传图片时，已经将图片的名字存在 editor.uploadImgOriginalName
				let img;
				if (resultText.indexOf('error|') === 0) {
					...
					img.onload = function () {
						let html = '<img src="' + resultText + '" alt="' + originalName + '" style="max-width:100%;"/>';
...
// xhr 上传图片
	_e(function (E, $) {
		...
		E.plugin(function () {
			...
			let config = editor.config;
			let uploadImgUrl = config.uploadImgUrl;
			let uploadTimeout = config.uploadTimeout;

			// 获取配置中的上传事件
			let uploadImgFns = config.uploadImgFns;
			let onload = uploadImgFns.onload;
			let ontimeout = uploadImgFns.ontimeout;
			let onerror = uploadImgFns.onerror;
			...
			editor.xhrUploadImg = function (opt) {
				...
				let base64 = opt.base64;
				let fileType = opt.fileType || 'image/png'; // 无扩展名则默认使用 png
				let name = opt.name || 'wangEditor_upload_file';
				let loadfn = opt.loadfn || onload;
				let errorfn = opt.errorfn || onerror;
				let timeoutfn = opt.timeoutfn || ontimeout;

				// 上传参数（如 token）
				let params = editor.config.uploadParams || {};

				// headers
				let headers = editor.config.uploadHeaders || {};

				// 获取文件扩展名
				let fileExt = 'png';  // 默认为 png
...
// 进度条
	_e(function (E, $) {
		...
		E.plugin(function () {
			...
			let editor = this;
			let menuContainer = editor.menuContainer;
			let menuHeight = menuContainer.height();
			let $editorContainer = editor.$editorContainer;
			let width = $editorContainer.width();
			let $progress = $('<div class="wangEditor-upload-progress"></div>');
...
// upload img 插件
	_e(function (E, $) {
		...
		E.plugin(function () {
			...
			// 获取editor的上传dom
			let $uploadContent = editor.$uploadContent;
			if (!$uploadContent) {
				return;
			}

			// 自定义UI，并添加到上传dom节点上
			let $uploadIcon = $('<div class="upload-icon-container"><i class="wangeditor-menu-img-upload"></i></div>');
			$uploadContent.append($uploadIcon);

			// ---------- 构建上传对象 ----------
			let upfile = new E.UploadFile({
				editor: editor,
...
// h5 方式上传图片
	_e(function (E, $) {
		...
		UploadFile.fn.render = function () {
			...
			let fileAccept = self.fileAccept;
			let acceptTpl = fileAccept ? 'accept="' + fileAccept + '"' : '';
			let multiple = self.multiple;
			let multipleTpl = multiple ? 'multiple="multiple"' : '';
			let $input = $('<input type="file" ' + acceptTpl + ' ' + multipleTpl + '/>');
			let $container = $('<div style="visibility:hidden;"></div>');
		...
		UploadFile.fn.upload = function (file) {
			...
			let uploadImgFns = editor.config.uploadImgFns;
			let uploadFileName = editor.config.uploadImgFileName || 'wangEditorH5File';
			let onload = uploadImgFns.onload;
			let ontimeout = uploadImgFns.ontimeout;
			let onerror = uploadImgFns.onerror;
			...
			reader.onload = function (e) {
				...
				let base64 = e.target.result || this.result;
				editor.xhrUploadImg({
					event: e,
					filename: filename,
					base64: base64,
...
// form方式上传图片
	_e(function (E, $) {
		...
		UploadFile.fn.render = function () {
			let self = this;
			let editor = self.editor;
			let uploadFileName = editor.config.uploadImgFileName || 'wangEditorFormFile';
			if (self._hasRender) {
				// 不要重复渲染
				return;
			}
			...
			// 创建 form 和 iframe
			let iframeId = 'iframe' + E.random();
			let $iframe = $('<iframe name="' + iframeId + '" id="' + iframeId + '" frameborder="0" width="0" height="0"></iframe>');
			let multiple = self.multiple;
			let multipleTpl = multiple ? 'multiple="multiple"' : '';
			let $p = $('<p>选择图片并上传</p>');
			let $input = $('<input type="file" ' + multipleTpl + ' name="' + uploadFileName + '"/>');
			let $btn = $('<input type="submit" value="上传"/>');
			let $form = $('<form enctype="multipart/form-data" method="post" action="' + uploadUrl + '" target="' + iframeId + '"></form>');
			let $container = $('<div style="margin:10px 20px;"></div>');
			...
			// 生成 modal
			let modal = new E.Modal(editor, undefined, {
				$content: $container
			});
			self.modal = modal;
		...
		UploadFile.fn.bindLoadEvent = function () {
			...
			let editor = self.editor;
			let $iframe = self.$iframe;
			let iframe = $iframe.get(0);
			let iframeWindow = iframe.contentWindow;
			let onload = editor.config.uploadImgFns.onload;

			// 定义load事件
			function onloadFn() {
				let resultText = $.trim(iframeWindow.document.body.innerHTML);
				if (!resultText) {
					return;
				}

				// 获取文件名
				let fileFullName = self.$input.val();  // 结果如 C:\folder\abc.png 格式
				let fileOriginalName = fileFullName;
...
// upload img 插件 粘贴图片
	_e(function (E, $) {
		...
		E.plugin(function () {
			...
			let config = editor.config;
			let uploadImgUrl = config.uploadImgUrl;
			let uploadFileName = config.uploadImgFileName || 'wangEditorPasteFile';
			let pasteEvent;
			let $imgsBeforePaste;
			...
			$txt.on('paste', function (e) {
				pasteEvent = e;
				let data = pasteEvent.clipboardData || pasteEvent.originalEvent.clipboardData;
				let text;
				let items;
...
// 拖拽上传图片 插件
	_e(function (E, $) {
		...
		E.plugin(function () {
			...
			let config = editor.config;
			let uploadImgUrl = config.uploadImgUrl;
			let uploadFileName = config.uploadImgFileName || 'wangEditorDragFile';
			...
			$txt.on('drop', function (dragEvent) {
				...
				let originalEvent = dragEvent.originalEvent;
				let files = originalEvent.dataTransfer && originalEvent.dataTransfer.files;
...
// 编辑器区域 table toolbar
	_e(function (E, $) {
		...
		E.plugin(function () {
			...
			let $currentTxt = editor.useMaxHeight ? $txt.parent() : $txt;
			let $currentTable;

			// 用到的dom节点
			let isRendered = false;
			let $toolbar = $('<div class="txt-toolbar"></div>');
			let $triangle = $('<div class="tip-triangle"></div>');
			let $delete = $('<a href="#"><i class="wangeditor-menu-img-trash-o"></i></a>');
			let $zoomSmall = $('<a href="#"><i class="wangeditor-menu-img-search-minus"></i></a>');
			let $zoomBig = $('<a href="#"><i class="wangeditor-menu-img-search-plus"></i></a>');
			...
			function show() {
				...
				let tablePosition = $currentTable.position();
				let tableTop = tablePosition.top;
				let tableLeft = tablePosition.left;
				let tableHeight = $currentTable.outerHeight();
				let tableWidth = $currentTable.outerWidth();

				// --- 定位 toolbar ---

				// 计算初步结果
				let top = tableTop + tableHeight;
				let left = tableLeft;
				let marginLeft = 0;

				let txtTop = $currentTxt.position().top;
				let txtHeight = $currentTxt.outerHeight();
...
// 编辑器区域 img toolbar
	_e(function (E, $) {
		...
		E.plugin(function () {
			...
			// 用到的dom节点
			let isRendered = false;
			let $dragPoint = $('<div class="img-drag-point-ss"></div>');
			...
			// let $zoomSmall = $('<a href="#"><i class="wangeditor-menu-img-search-minus"></i></a>');
			// let $zoomBig = $('<a href="#"><i class="wangeditor-menu-img-search-plus"></i></a>');
			// let $floatLeft = $('<a href="#"><i class="wangeditor-menu-img-align-left"></i></a>');
			// let $noFloat = $('<a href="#"><i class="wangeditor-menu-img-align-justify"></i></a>');
			// let $floatRight = $('<a href="#"><i class="wangeditor-menu-img-align-right"></i></a>');
			let $alignLeft = $('<a href="#"><i class="wangeditor-menu-img-align-left"></i></a>');
			let $alignCenter = $('<a href="#"><i class="wangeditor-menu-img-align-center"></i></a>');
			let $alignRight = $('<a href="#"><i class="wangeditor-menu-img-align-right"></i></a>');
			let $link = $('<a href="#"><i class="wangeditor-menu-img-link"></i></a>');
			let $unLink = $('<a href="#"><i class="wangeditor-menu-img-unlink"></i></a>');

			let $linkInputContainer = $('<div style="display:none;"></div>');
			let $linkInput = $('<input type="text" style="height:26px; margin-left:10px; width:200px;"/>');
			let $linkBtnSubmit = $('<button class="right">' + lang.submit + '</button>');
			let $linkBtnCancel = $('<button class="right gray">' + lang.cancel + '</button>');
			...
			function bindDragEvent() {
				let _x, _y;
				let dragMarginLeft, dragMarginTop;
				...
				function mousemove(e) {
					...
					// --------- 计算拖拽点的位置 ---------
					let currentDragMarginLeft = dragMarginLeft + diffX;
					let currentDragMarginTop = dragMarginTop + diffY;
					$dragPoint.css({
			...
			function show() {
				...
				let imgPosition = $currentImg.position();
				let imgTop = imgPosition.top;
				let imgLeft = imgPosition.left;
				let imgHeight = $currentImg.outerHeight();
				let imgWidth = $currentImg.outerWidth();
...
// 编辑区域 link toolbar
	_e(function (E, $) {
		E.plugin(function () {
			...
			let $toolbar = $('<div class="txt-toolbar"></div>');
			let $triangle = $('<div class="tip-triangle"></div>');
			let $triggerLink = $('<a href="#" target="_blank"><i class="wangeditor-menu-img-link"></i> ' + lang.openLink + '</a>');
			let isRendered;
			...
			let showTimeoutIdByToolbar, hideTimeoutIdByToolbar;
			...
			function setPosition() {
				...
				// 初步计算top值
				let topResult = top + height + 5;

				// 判断 toolbar 是否超过了编辑器区域的下边界
				let menuHeight = editor.menuContainer.height();
				let txtHeight = editor.txt.$txt.outerHeight();
...
// menu吸顶
	_e(function (E, $) {
		...
		E.plugin(function () {
			let editor = this;
			let menuFixed = editor.config.menuFixed;
			if (menuFixed === false || typeof menuFixed !== 'number') {
				// 没有配置菜单吸顶
				return;
			}
			let bodyMarginTop = parseFloat(E.$body.css('margin-top'), 10);
			if (isNaN(bodyMarginTop)) {
				bodyMarginTop = 0;
			}

			let $editorContainer = editor.$editorContainer;
			let editorTop = $editorContainer.offset().top;
			let editorHeight = $editorContainer.outerHeight();

			let $menuContainer = editor.menuContainer.$menuContainer;
			let menuCssPosition = $menuContainer.css('position');
			let menuCssTop = $menuContainer.css('top');
			let menuTop = $menuContainer.offset().top;
			let menuHeight = $menuContainer.outerHeight();
			...
			E.$window.scroll(function () {
				...
				let menuWidth = $menuContainer.width();
	...
	_e(function (E, $) {
		...
		E.createMenu(function (check) {
			...
			editor.commandHooks.lineHeight = function (value) {
				let rangeElem = editor.getRangeElem();
				let targetElem = editor.getSelfOrParentByName(rangeElem, 'p,h1,h2,h3,h4,h5,pre');
				if (!targetElem) {
					return;
				}
			...
			// 为menu创建droplist对象
			let tpl = '<span style="line-height:{#commandValue}">{#title}</span>';
			menu.dropList = new E.DropList(editor, menu, {
				data: data,  // 传入数据源
				tpl: tpl  // 传入模板
			});
...
// 自定义上传
	_e(function (E, $) {
		...
		E.plugin(function () {
			...
			let editor = this;
			let customUpload = editor.config.customUpload;
			if (!customUpload) {
				return;
			...
			// 设置id，并暴露
			let btnId = 'upload' + E.random();
			let containerId = 'upload' + E.random();
