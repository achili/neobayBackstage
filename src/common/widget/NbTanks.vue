<template>
	<div class="nb-tanks-block">
		...
		<div class="tank-block" v-for="(tank,tankIndex) in tanks">
			...
			<NbExpanding>
				<div v-show="tank.mode===tank.Mode.TINY && tank.procedure === tank.Procedure.UPLOADING">
					<i class="fa fa-spinner fa-spin fa-fw"></i> 正在上传中...
			...
			<NbExpanding>
				<div class="huge-block clearfix"
				     v-if="tank.mode===tank.Mode.HUGE && (tank.procedure === tank.Procedure.UPLOADING)">
					<div class="clearfix">
						<h4 class="pull-left">{{tank.file.name}}</h4>
						<i class="pull-right btn-action f16 fa fa-trash text-danger" @click.stop.prevent="del(tank,tankIndex)"></i>
					</div>
					...
					<div>
						已上传: {{ (tank.file.size * tank.progress).toFixed(0) | humanFileSize }} / {{ tank.file.size | humanFileSize}} 当前上传速度：{{tank.speed | humanFileSize}}/s
